...
echo "======================================"
echo "실습 6: YOLOv11 세그멘테이션 전이학습"
echo "픽셀 단위 객체 분할 모델 구현"
echo "======================================"
echo ""
echo "실습 목표:"
echo "- Isaac Sim Instance Segmentation 데이터 활용"
echo "- YOLOv11n-seg 모델 전이학습"
echo "- 픽셀 단위 마스크 생성 및 평가"
echo ""
echo "======================================"
...
while [[ $# -gt 0 ]]; do
    case $1 in
        ...
        --help)
            ...
            echo "  $0 --inference-only --weights runs/segment/yolo11_seg_isaac_*/weights/best.pt"
            echo ""
            exit 0
            ;;
        *)
...
# 실행 명령 구성
CMD="python3 yolo_seg_training.py"
...
if [ $RESULT -eq 0 ]; then
    ...
    # 결과 파일 확인
    if [ -f "segmentation_result.jpg" ]; then
        echo ""
        echo "생성된 파일:"
        echo "  - segmentation_result.jpg (추론 결과)"
    fi
    
    if [ -d "yolo_seg_dataset" ]; then
        echo "  - yolo_seg_dataset/ (변환된 데이터셋)"
    fi
    
    if [ -d "runs/segment" ]; then
        echo "  - runs/segment/ (학습 결과)"
    fi
else
